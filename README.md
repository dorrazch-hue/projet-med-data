...
Contient les variables d'environnement (connexion, noms des collections)

docker-compose.yml

Définit les services (MongoDB, Mongo Express, App)

migration.py

Script principal de migration des données du CSV vers MongoDB

healthcare_dataset.csv

Jeu de données médicales à importer

validate.py

Vérifie et nettoie les données avant la migration
...
Le dashboard mongo-express est accessible sur : http://localhost:8081
...
Chaîne de connexion utilisée par le script migration.py

ME_CONFIG_MONGODB_AUTH_DATABASE

Base ciblée par Mongo Express (medical_db)
...
Pour sécuriser l'accès et permettre une évaluation externe, vous pouvez créer un utilisateur avec des droits spécifiques :
...
Typage : dates -> datetime, nombres -> int/float, booléens, None.

Index : ajout des index dédiés aux filtres fréquents du client.

Scalabilité : insertion par chunks, ordered=False, gestion des timeouts.

10. Déploiement AWS (Perspectives)

Ce projet est conçu pour être facilement porté sur l'infrastructure Amazon Web Services (AWS) :

Amazon S3 : Stockage sécurisé et durable des fichiers sources CSV.

Amazon DocumentDB : Base NoSQL managée compatible avec l'API MongoDB.

Amazon ECS / Fargate : Exécution du script de migration en mode conteneurisé "Serverless".

AWS IAM : Gestion fine des accès et de la sécurité des données.
